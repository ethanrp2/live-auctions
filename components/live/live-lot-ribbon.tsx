...
function StatusChip({
  ...
  if (status === "live") {
    return (
      <span
        className="inline-flex items-center gap-1 rounded-[2px] bg-[rgba(255,0,4,0.4)] px-1.5 py-0.5 text-[10px] uppercase tracking-[-0.02em] text-white"
        style={{ fontFamily: "var(--storefront-font-mono)" }}
      >
        <span className="h-2 w-2 rounded-full bg-[#ff0004]" aria-hidden="true" />
        Live
      </span>
    );
  }
        ...
        {lots.map((lot, index) => {
          ...
          return (
            ...
            >
              ...
              <StatusChip status={lot.liveStatus} isNext={isNext} />
            </button>
          );
        })}
      </div>
